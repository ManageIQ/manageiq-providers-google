describe ManageIQ::Providers::Google::CloudManager::ProvisionWorkflow do
  ...
  let(:template) { FactoryBot.create(:template_google, :name => "template", :ext_management_system => ems) }
  let(:workflow) do
    stub_dialog
  ...
  context "availability_zone_to_cloud_network" do
    it "has one when it should" do
      FactoryBot.create(:cloud_network_google, :ext_management_system => ems.network_manager)
